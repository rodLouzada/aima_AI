...
public class GoldBoard {
	...
	public GoldBoard() {
		state = new int[] { 1, 0, 0, 0, 0, 0, 0, 2, 0 };
	}

	public GoldBoard(int[] state) {
		this.state = new int[state.length];
		System.arraycopy(state, 0, this.state, 0, state.length);
	}

	public GoldBoard(GoldBoard copyBoard) {
	...
	public void moveGoldRight() {
		int AgentPos = getAgentPosition();
		int x = getXCoord(AgentPos);
		int ypos = getYCoord(AgentPos);
		if (!(ypos == 2)) {
	...
	public void moveGoldDown() {
		int AgentPos = getAgentPosition();
		int x = getXCoord(AgentPos);
		int y = getYCoord(AgentPos);
		if (!(x == 2)) {
	...
	public boolean canMoveGold(Action where) {
		boolean retVal = true;
		int absPos = getPositionOf(1);
